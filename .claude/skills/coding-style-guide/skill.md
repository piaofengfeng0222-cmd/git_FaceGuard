...
description: 约定C++语言的编程风格，比如命名、格式、注释、头文件、作用域、类等
---
...
C++的非类文件的头文件和cpp文件名保持一致，使用大驼峰命名风格。

如果有一个头文件Utils.h，那么对应的cpp文件名：

- Utils.cpp

结构体，命名空间，枚举等定义的文件名类似。

### <a name="r2-2-3"></a>规则2.2.3 C++类文件名使用大驼峰命名风格

C++的头文件和cpp文件名保持一致，使用大驼峰命名风格。
...
### <a name="r2-5-1"></a>规则2.5.1 变量命名规则：

1. 用最短字符表示最准确的意义。
2. 使用变量前缀。

#### <a name="r2-5-2-1"></a>2.5.2.1整型前缀
...
#### <a name="r2-5-2-1"></a>2.5.2.2浮点型前缀

　　float fValue;　　　　　　 //float前缀：f
　　double dValue; 　　　 //double前缀：d

#### <a name="r2-5-2-1"></a>2.5.2.3字符型前缀

　　char chChar;　　　　　　//char前缀：ch

#### <a name="r2-5-2-1"></a>2.5.2.4字符串前缀

　　char szPath; 　　　　 //char字符串前缀：sz
　　string strPath; 　　　 //string字符串前缀：str
　　CString strPath; 　　 //MFC CString类前缀：str

#### <a name="r2-5-2-1"></a>2.5.2.5布尔型前缀

　　bool bIsOK; 　　　 //bool类型前缀：b
　　BOOL bIsOK; 　　　 //MFC BOOL前缀：b

#### <a name="r2-5-2-1"></a>2.5.2.6 指针型前缀

　　char* pPath; 　 　 //指针前缀：p

#### <a name="r2-5-2-1"></a>2.5.2.7数组前缀

　　int arrnNum[]; 　　　 //数组前缀：arr
　　CString arrstrName[]; //数组前缀+类型前缀+名称

#### <a name="r2-5-2-1"></a>2.5.2.8结构体前缀

　　STUDENT tXiaoZhang; //结构体前缀：t

#### <a name="r2-5-2-1"></a>2.5.2.9枚举前缀

　　enum emWeek; 　 //枚举前缀：em

#### <a name="r2-5-2-1"></a>2.5.2.10字节的前缀

　　BYTE byIP; 　　　　　 //字节前缀：by

#### <a name="r2-5-2-1"></a>2.5.2.11字的前缀

　　DWORD dwMsgID; 　 //双字前缀：dw
　　WORD wMsgID; 　 //单字前缀：w

#### <a name="r2-5-2-1"></a>2.5.2.12字符指针前缀

　　LPCTSTR ptszName; //TCHAR类型为ptsz
　　LPCSTR pszName; //pcsz
　　LPSTR pszName; 　 //psz

#### <a name="r2-5-2-1"></a>2.5.2.13 STL容器前缀

　　vector 	vecValue; 　 //vector容器前缀：vec

　　map 	   mapValue;   //map容器前缀：map

　　list 	      lstInfo;	 //map容器前缀：list

#### <a name="r2-5-2-1"></a>2.5.2.14 RECT矩形结构前缀

　　RECT rcChild; 　　　 //rc
　　CRECT rcChild; 　　 //rc

#### <a name="r2-5-2-1"></a>2.5.2.15 句柄前缀
...
#### <a name="r2-5-2-1"></a>2.5.2.16 Windows颜色前缀

　　COLORREF crFont; //cr

#### <a name="r2-5-2-1"></a>2.5.2.17 Windows DC前缀

　　CDC dcClient; 　　 //dc

### <a name="r2-5-2"></a>类的成员变量以m_开头，后面为变量，变量同时还要加前缀。

　　CString m_strName; //m_开头+类型前缀+名称

### <a name="r2-5-3"></a>定义一个变量，为了简化，在不影响变量意义的情况下，可仅仅使用前缀。

　　RECT rc;

### <a name="r2-5-4"></a>全局变量一律以g_开头，后面为变量，变量同时还要加前缀。

　　int g_ID; //g

### <a name="r2-5-5"></a>定义结构体，保证C和C++兼容，采用typedef语句，并且结构体类型全部大写，以ST_开头，指针形式以PT_开头。
...
　　　　CString strName;
　　}STUDENT, *PSTUDENT;
　　STUDENT stXiaoZhang; 　　　//完整定义结构体

### <a name="r2-5-6"></a>变量由多个单词组成，则每个单词的首个字母大写。

　　int nStudentID;
　　CString strStudentName;

### <a name="r2-5-7"></a>定义一个类以C做为类名前缀。

　　class CMyListCtrl;

### <a name="r2-5-8"></a>控件ID尽量简化并表明控件类型和意义。
　　Button IDC_BNT_NAME;
　　Edit IDC_EDT_NAME;
　　ListBox IDC_LB_NAME;
　　ListCtrl IDC_LC_NAME;

### <a name="r2-5-9"></a>全局变量应增加 'g_' 前缀，静态变量命名不需要加特殊前缀
